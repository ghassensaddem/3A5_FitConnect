...
public class SeanceService implements IService<seance> {
    ...
    public void ajouter(seance seance) {
        String req = "INSERT INTO seance (date, horaire, lieu, programme_id) VALUES (?, ?, ?, ?)";
        try {
    ...
    public void modifier(seance seance) {
        String req = "UPDATE seance SET date=?, horaire=?, lieu=?, programme_id=? WHERE id=?";
        try {
            PreparedStatement pst = connection.prepareStatement(req);
