...
public class ClientService implements iService<Client> {
    ...
    public void ajouter(Client client) {
        String req = "INSERT INTO client (nom,prenom,sexe,mdp,dateNaissance,email,poids,taille,objectif,idEvent,id_prog) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
        try {
    ...
    public void modifier(Client client) {
        String req = "UPDATE client SET nom=? ,prenom=?,sexe=?,mdp=?,DateNaissance=?,email=?,poids=?,taille=?,objectif=?,idEvent=?,id_prog=? WHERE id=?";
        try {
            PreparedStatement pst = connection.prepareStatement(req);
            pst.setString(1, client.getNom());
            pst.setString(2, client.getPrenom());
    ...
    public ArrayList<Client> rechercher() {
        ...
        try {
            ...
            while (rs.next()) {
                clients.add(new Client(rs.getInt("id"), rs.getString("nom"), rs.getString("prenom"), rs.getString("sexe"),rs.getString("mdp"),rs.getString("dateNaissance"),rs.getString("email"),rs.getFloat("poids"),rs.getFloat("taille"),rs.getString("objectif"),rs.getInt("idEvent"),rs.getInt("id_Prog")));
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
