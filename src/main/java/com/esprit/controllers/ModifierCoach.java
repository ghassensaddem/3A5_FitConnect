...
public class ModifierCoach{
    ...
    @FXML
    private LineChart<String, Number> programmeChart;
    @FXML
    ...
    public void initialize() {
        ...
        series.setName("Évolution des Applications");
        programmeChart.getData().add(series);
        Platform.runLater(() -> {
            if (series.getNode() != null) {
                series.getNode().getStyleClass().add("coach-series-0");
            }
        });
        xAxis.setLabel("Mois");
        yAxis.setLabel("Nombre d'Applications");
        afficherStatistiques(id);
    }
    ...
    void Update(ActionEvent event) throws IOException {
        if(validateFields()){
            ...
            cs.modifier(new Coach(id,nom.getText(),prenom.getText(), sexe,cryptoUtils.encrypt(mdp.getText()),DateNaissance.getValue().toString(),email.getText(),pdpPathField.getText(),lieuEngagement.getValue(),specialite.getValue()));
    ...
    public void afficherStatistiques(int coachId) {
        CoachService coachService = new CoachService();
        XYChart.Series<String, Number> series = new XYChart.Series<>();
        series.setName("Évolution des Applications");

        // Récupération des données depuis CoachService
        Map<String, Integer> stats = coachService.getStatistiquesApplicationsParMois(coachId);
