...
public class ModifierClient {
    ...
    public PieChart commandeChart;
    public PieChart applicationChart;
    @FXML
    ...
    public void Update(ActionEvent actionEvent) throws IOException {
        ...
        if(checkFemme.isSelected())
            sexe="femme";
        cs.update(new Client(id,nom.getText(),prenom.getText(), sexe,cryptoUtils.encrypt(mdp.getText()),DateNaissance.getValue().toString(),email.getText(),pdpPathField.getText(),poids.getValue().floatValue(),taille.getValue().floatValue()));
    ...
    private void chargerStatistiques(int clientId) {
        ClientService clientService = new ClientService();
        // Récupération des données
        int totalCommandes = clientService.getTotalCommandes();
        int commandesClient = clientService.getCommandesByClient(clientId);
        int totalApplications = clientService.getTotalApplications();
        int applicationsClient = clientService.getApplicationsByClient(clientId);
        System.out.println("Total commandes : " + totalCommandes);
        System.out.println("Commandes client : " + commandesClient);
        System.out.println("Total applications : " + totalApplications);
        System.out.println("Applications client : " + applicationsClient);


        // Création des données pour le PieChart des commandes
        ObservableList<PieChart.Data> commandeData = FXCollections.observableArrayList(
                new PieChart.Data("Commandes Client", commandesClient),
                new PieChart.Data("Autres Commandes", totalCommandes - commandesClient)
        );
        commandeChart.setData(commandeData);

        // Création des données pour le PieChart des applications
        ObservableList<PieChart.Data> applicationData = FXCollections.observableArrayList(
                new PieChart.Data("Applications Client", applicationsClient),
                new PieChart.Data("Autres Applications", totalApplications - applicationsClient)
        );
        applicationChart.setData(applicationData);
    }
