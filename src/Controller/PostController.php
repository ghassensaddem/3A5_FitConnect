...
public function ajouter(Request $request, EntityManagerInterface $em): Response
{
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        if ($imageFile) {
            ...
            // Déplacer le fichier dans le répertoire public/assets/images
            try {
                $imageFile->move(
                    $this->getParameter('kernel.project_dir') . '/public/images', // Chemin absolu vers le répertoire
                    $newFilename
                );
...
public function showComments_front(PostRepository $postRepository, CommentaireRepository $commentaireRepository, Request $request,EntityManagerInterface $em, int $id): Response {
    ...
    $commentaire = new Commentaire();
    $client = $em->find(Client::class, 8);  // À remplacer par l'utilisateur connecté
    $commentaire->setClient($client);
    $commentaire->setPost($post);
...
#[Route('/post/edit/{id}', name: 'app_post_edit')]
    public function edit(int $id, Request $request, EntityManagerInterface $em): Response
    {
        ...
        // Créer le formulaire avec les données du post existant
        $form = $this->createForm(PostType::class, $post);
...
public function vote(
    ...
    try {
        ...
        $em->persist($post);
        $em->flush();
dd($post->getUpvotes(), $post->getDownvotes());
